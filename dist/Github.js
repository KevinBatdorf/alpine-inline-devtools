(function (factory) {
    ...
    const DevTools = {
      ...
      injectButtonIntoPage() {
        ...
        const styleSheet = document.createElement('style'); // Force the opacity when the button is open. I noticed TailwindUI is messing with this otherwise, for example
    ...
    const Loader = function (Viewer, theme) {
      return {
        ...
        openWindow() {
          ...
          if (!this.windowRef) return sessionStorage.removeItem('alpine-devtools'); // Starting color. Consider some loading animation
          ...
          this.windowRef.document.head.appendChild(devtoolsTheme); // Add the dev tools component function and remove any previous
        ...
        setUpPopupData() {
          ...
          viewer.innerHTML = this.viewerShell(); // Add the reference to the session so we don't need to reopen the popup everytime
